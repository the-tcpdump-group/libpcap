...
pcap_activate_hurd(pcap_t *p)
{
	...
	/* Try devnode first */
	master = file_name_lookup(p->opt.source, O_READ | O_WRITE, 0);
	...
	if (master != MACH_PORT_NULL)
		...
		kr = device_open(master, D_READ | D_WRITE, p->opt.source,
				 &ph->mach_dev);
	}
...
pcap_create_interface(const char *device, char *ebuf)
{
	...
	p = pcap_create_common(device, ebuf, sizeof (struct pcap_hurd));
